...
# 구간 끝을 다음 phase 시작까지 늘려 사이 공백을 없앴다 — GitHub Actions cron이
# 관측상 최대 2~4.5시간까지 지연되는 탓에, 예전처럼 open을 90분(09:30~11:00)으로
# 좁게 잡으면 지연된 실행이 도착했을 땐 이미 구간 밖이라 그날 open이 통째로
# 영구 스킵되는 일이 실제로 몇 주째 반복됐다(2026-07-06 이후 open 캡처 0건).
# 이제는 늦게 도착해도 아직 그날 못 잡은 phase면(그리고 다음 phase 시작 전이면)
# 그걸로 캡처한다 — 정시 발화 시엔 기존과 동일하게 동작.
WINDOWS = {
    "open": ((9, 30), (11, 30)),
    "mid": ((11, 30), (15, 35)),
    "close": ((15, 35), (23, 30)),
