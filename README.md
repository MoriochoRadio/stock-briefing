...
- **Market Focus** — 히어로 아래에서 국내 반도체의 기술 지표·미국 반도체 흐름·시장 심리를 세 장의 관찰 카드로 요약. 별표로 고정한 종목은 이 브라우저에만 저장해 빠르게 필터링
- **반도체 상대 강도** — 삼성전자와 SK하이닉스를 동일 기준점(100)으로 정규화하여 1개월·3개월·6개월·1년 성과 차이를 정확히 비교
- **나의 관찰 데스크** — 수집 대상 안에서 최대 8개 종목을 브라우저에만 저장해 개인 데스크로 구성. 5·10·20개 기록 성과와 설정한 당일 변동폭 기준을 함께 비교하며, 서버 계정·개인정보·푸시 알림은 사용하지 않음
- **Market Replay** — AI 컴퓨팅·메모리·제조·장비 종목군을 해당 기간의 첫 종가 `100`으로 정규화해 1개월·3개월·6개월·1년 가격 변화의 상대적 흐름을 비교
- **Ecosystem Pulse** — 미국 AI·제조장비·글로벌 메모리·국내 메모리 종목군의 일간 등락률을 한 화면에서 비교. 연결선은 인과관계가 아닌 관찰 경로이며, 수집된 종목만 단순 평균으로 반영
- **아카이브 시장 기록 비교** — 아카이브에서 두 수집 날짜를 직접 선택해 코스피·나스닥·AI 컴퓨팅·메모리 주요 종목의 종가 변화와 변화폭 상위를 회고. 날짜별 브리핑 해석과 가격 기록을 분리해 검토
- **나의 기록 렌즈** — 개인 데스크의 브라우저 저장 관심종목을 아카이브로 연결해, 같은 두 수집일의 종가 변화만 따로 비교. 설정은 서버에 전송하지 않음
- **브리핑·뉴스 변화 비교** — 구조화된 주제 언급과 보존된 이벤트 헤드라인을 기준으로 새로 부각·반복·약화된 항목을 표시. 중요도·사실성·원인·예측을 자동 판정하지 않음
- **관찰 규칙 라이브러리** — 개인 관심종목의 당일 변동폭·기간 모멘텀·방향 일치 여부를 화면에서만 조합해 확인. 자동 매매·알림·성과 예측 없음
- **이벤트 기록 타임라인** — 수집 시점의 기사 카테고리·출처·링크를 일자별로 누적. 기능 도입일 이후부터 기록을 쌓으며 가격 인과관계를 주장하지 않음
- **주간·월간 회고** (`/reviews`) — 수집 종가 첫·마지막 값의 단순 비교와 당시 이벤트 기록으로 결정론적으로 생성. 매일 최신 기간을 갱신
- **인텔리전스 맵 UI** — Hero 아래 빠른 탐색 레일과 `전체/핵심만` 밀도 전환을 제공. 읽기 모드는 브라우저에만 저장
- **대시보드** (`/dashboard`) — 벤토 그리드, 섹터 히트맵 · **아카이브** — 날짜별 과거 브리핑

## 동작 구조

**스크립트** (`scripts/`)
- `fetch_data.py` — yfinance 시세 + Google News RSS + CNN 공포·탐욕지수 수집 → `sentiment/series/news/history.json`, `event_ledger.json`, `quality.json`(수집 완전성·기준일·출처)
- `generate_reviews.py` — 가격 수집 스냅샷의 첫·마지막 종가와 이벤트 기록만 사용해 `reviews/weekly`, `reviews/monthly` 회고를 결정론적으로 생성
- `seed_event_ledger.py` — 기존 공개 뉴스 스냅샷으로 이벤트 기록을 최초 초기화하는 일회성 스크립트
...
- `daily.yml` — 22:10 UTC(07:10 KST) 모닝 브리핑. 데이터·브리핑 생성 후 현재 주간·월간 회고와 이벤트 기록도 함께 갱신
